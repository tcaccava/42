...
/*   Updated: 2026/05/02 23:08:12 by aseveri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	if (init_gnl(&buffer, &str, temp))
		return (str);
	while (1)
	{
		n_byte = read(fd, buffer, BUFFER_SIZE);
		if (n_byte < 0)
			return (free_and_null(&buffer), free_and_null(&str), NULL);
		if (!n_byte)
			return (free_and_null(&buffer), str);
		buffer[n_byte] = '\0';
		if (!str)
		{
			if (read_no_str(&buffer, &str, temp))
				return (str);
		}
		else if (read_with_str(&buffer, &str, temp))
			return (str);
	}
}
