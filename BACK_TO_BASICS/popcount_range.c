/**
 * =====================================================================================
 * @file        bit_range_count.c
 * @brief       Conteggio selettivo di bit in un registro a 32-bit.
 *
 * 1. BACKGROUND TEORICO:
 *    Il problema consiste nell'isolare un sottoinsieme contiguo di bit (range)
 *    all'interno di una word a 32 bit per poi applicarne il peso di Hamming.
 *    L'approccio qui implementato utilizza maschere dinamiche costruite tramite
 *    shift aritmetici, garantendo un'operazione branchless (priva di salti condizionali) 
 *    nella fase di selezione, seguita da un calcolo del popcount in tempo costante
 *    (O(1)) tramite la tecnica SWAR.
 *
 * 2. APPLICAZIONI PRATICHE:
 *    - Networking: Parsing di pacchetti IP dove i campi sono spesso non allineati ai byte.
 *    - Bit-Field Manipulation: Gestione di stati di macchine a stati finite (FSM) 
 *      compresse in registri.
 *    - Cache-line profiling: Analisi di flag di stato in strutture di controllo.
 *
 * 3. IMPLEMENTAZIONE HARDWARE:
 *    Il codice sfrutta le proprietà algebriche dell'ALU (XOR swap, shift logici).
 *    L'utilizzo di `~0U` (complemento a uno di zero) garantisce la massima portabilità
 *    nel generare una maschera di tutti '1' (0xFFFFFFFF) indipendentemente dall'architettura.
 * =====================================================================================
 */
